...
const ToastViewport = forwardRef<ElementRef<typeof ToastPrimitives.Viewport>, ComponentPropsWithoutRef<typeof ToastPrimitives.Viewport>>(
  ({ className, ...props }, ref) => (
    <ToastPrimitives.Viewport ref={ref} className={cn("fixed bottom-0 right-0 top-auto z-[100] flex max-h-screen w-full flex-col p-4 md:max-w-[420px]", className)} {...props} />
  )
);
...
const toastVariants = cva(
  ...
  { variants: { variant: { default: "border bg-background text-foreground", destructive: "destructive group border-destructive bg-destructive text-destructive-foreground", success: "bg-[#ecfdf3] text-[#008a2e] border-[#d3fde5] dark:bg-[#00150a] dark:text-[#3ba16e] dark:border-[#002a13]" } }, defaultVariants: { variant: "default" } }
);
